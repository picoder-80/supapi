...
export async function GET(
  ...
  try {
    ...
    const queries = [
      ...
      supabase.from("listings").select("id", { count: "exact", head: true }).eq("seller_id", uid).eq("status", "active").eq("category", "software-digital"),
      supabase.from("classified_listings").select("id", { count: "exact", head: true }).eq("seller_id", uid).eq("status", "active"),
