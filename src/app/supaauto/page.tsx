...
export default function MachinaMarketPage() {
  ...
  const fetchHome = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(`/api/supaauto?${params}`);
  ...
  const handleSave = async (listingId: string) => {
    ...
    try {
      const r = await fetch("/api/supaauto", {
        method: "POST",
      ...
      <div className={styles.hero}>
        ...
        <div className={styles.heroContent}>
          ...
          <h1 className={styles.heroTitle}>
            <span className={styles.heroTitleMain}>Supa</span><span className={styles.heroTitleAccent}>Auto</span>
          </h1>
