...
export async function POST(req: NextRequest) {
  ...
  const pkg = AUTOREPOST_PACKAGES.find((row) => row.id === String(package_id ?? ""));
  ...
  try {
    ...
    await supabase.from("credit_transactions").insert({
      user_id: userId,
      type: "spend",
      activity: "market_autorepost",
      amount: -pkg.sc,
      balance_after: newBalance,
      note: `🔁 Market auto-repost: ${listing.title} (${pkg.label})`,
    });
