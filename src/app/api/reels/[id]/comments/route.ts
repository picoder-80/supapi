...
export async function POST(
  ...
  try {
    ...
    const { data: comment, error } = await supabase
      .from("feed_comments")
      .insert({ target_type: "reel", target_id: id, user_id: payload.userId, body: text })
      .select(`
        id, body, created_at,
        user:users!user_id(id, username, display_name, avatar_url)
      `)
    ...
    const { data: row } = await supabase.from("reels").select("comment_count").eq("id", id).single();
    const count = ((row as { comment_count?: number })?.comment_count ?? 0) + 1;
    await supabase.from("reels").update({ comment_count: count }).eq("id", id);
