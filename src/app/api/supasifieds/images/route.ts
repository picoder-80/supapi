...
export async function POST(req: NextRequest) {
  try {
    ...
    const classifiedId = formData.get("classified_id") as string | null;
    ...
    const folder = classifiedId ?? `draft_classified_${payload.userId}`;
    const path = `${payload.userId}/classified/${folder}/${index ?? Date.now()}.${ext}`;
