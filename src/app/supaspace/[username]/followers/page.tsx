...
export default function FollowersPage() {
  ...
  useEffect(() => {
    fetch(`/api/supaspace/follow-list/${encodeURIComponent(username)}?type=followers`)
      .then(r => r.json())
      .then(d => {
  ...
  return (
    <div className={styles.page}>
      <div className={styles.followListHeader}>
        ...
        <h1 className={styles.followListTitle}>@{username}'s Followers</h1>
        <p className={styles.followListSub}>{users.length} {users.length === 1 ? "follower" : "followers"}</p>
      </div>
      <div className={styles.body}>
        {loading ? (
          ...
          <div className={styles.empty}>
            ...
            <div className={styles.emptyDesc}>When people follow @{username}, they will appear here.</div>
