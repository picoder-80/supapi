...
export default function DomusPage() {
  ...
  const fetchHome = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(`/api/supadomus?${params}`);
  ...
  const handleRegisterAgent = async () => {
    ...
    try {
      const r = await fetch("/api/supadomus", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token()}` },
    ...
    <div className={styles.page}>
      ...
      <div className={styles.hero}>
        ...
        <div className={styles.heroContent}>
          <div className={styles.heroBadge}>🏠 SupaDomus — Pi Property</div>
      ...
      <div className={styles.controlBar}>
        <div className={styles.controlLeft}>
          ...
          <Link href="/supadomus/create" className={styles.listBtn}>+ List Property</Link>
        </div>
        ...
        {activeMode === "project" ? (
          <div className={styles.projectsList}>
            {projects.length === 0 ? (
              <div className={styles.empty}>
                ...
                <div className={styles.emptyDesc}>Be the first developer to launch on SupaDomus!</div>
              </div>
            ) : projects.map(proj => (
          ...
          browsing ? (
            ...
            <div className={styles.empty}>
              ...
              <Link href="/supadomus/create" className={styles.emptyBtn}>+ List Property</Link>
            </div>
          ) : (
            <div className={styles.grid}>
              {displayListings.map((listing: Listing) => {
                ...
                return (
                  <div key={listing.id} className={styles.listingCard}>
                    <Link href={`/supadomus/${listing.id}`} className={styles.listingImgWrap}>
                      {listing.images?.[0]
                    ...
                    <Link href={`/supadomus/${listing.id}`} className={styles.listingBody}>
        ...
        {topAgents.length > 0 && (
          <div className={styles.section}>
            <div className={styles.sectionHeader}>
              ...
              <Link href="/supadomus/agents" className={styles.sectionMore}>See All →</Link>
            </div>
            <div className={styles.agentsRow}>
              {topAgents.map((agent: Agent) => (
                <Link key={agent.id} href={`/supadomus/agent/${agent.id}`} className={styles.agentCard}>
