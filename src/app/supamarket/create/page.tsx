...
export default function CreateListingPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const r = await fetch("/api/supamarket/listings", {
        method: "POST",
      ...
      const d = await r.json();
      if (d.success) router.push(`/supamarket/${d.data.id}`);
  ...
  return (
    <div className={styles.page}>
      <div className={styles.topBar}>
        <button className={styles.iconBtn} onClick={() => router.back()} aria-label="Back">←</button>
        <h1 className={styles.title}>Create Listing</h1>
        <button className={styles.iconBtn} type="button" aria-label="Share" disabled>⤴</button>
      </div>
      ...
      <div className={styles.layout}>
                ...
                value={form.description} onChange={e => set("description", e.target.value)} />
                  ...
                  {error && (
                    <div className={styles.error}>
                      <div>{error}</div>
                      <button type="button" className={styles.errorBackBtn} onClick={() => router.back()}>Back</button>
                    </div>
                  )}
              ...
              <div className={styles.formCard}>
                <div className={styles.form}>
                  ...
                  <div className={styles.imgGrid}>
                    ...
                    {uploading && (
                      <div className={styles.uploadSkeleton} aria-hidden />
                    )}
        ...
        <aside className={styles.rightCol}>
          <div className={styles.infoCard}>
            <div className={styles.infoTitle}>Quick checklist</div>
            <ul className={styles.infoList}>
              <li>Use clear title and real photos</li>
              <li>Set fair price in Pi</li>
              <li>Add location and shipping method</li>
              <li>Complete preview before publish</li>
            </ul>
          </div>
          <div className={styles.infoCard}>
            <div className={styles.infoTitle}>Escrow protection</div>
            <p className={styles.infoText}>
              Pi held in escrow until buyer confirms delivery.
            </p>
          </div>
        </aside>
      </div>
      ...
      <div className={styles.stickyBottomBar}>
        <div className={styles.stickyPriceWrap}>
          <div className={styles.stickyLabel}>Preview price</div>
          <div className={styles.stickyPrice}>{parseFloat(form.price_pi || "0").toFixed(2)} π</div>
        </div>
        {step !== "preview" ? (
          <button
          ...
          >
            {step === "details" ? "Next: Photos" : "Next: Preview"}
          </button>
        ) : (
