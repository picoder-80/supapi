...
export async function POST(req: NextRequest, { params }: Params) {
  try {
      ...
      try {
        if (buyer.email) {
          await sendPlatformEmail({
            ...
            html: `<p>Your dispute for Order #${order.id} has been resolved. ${amountPi.toFixed(2)}π has been sent back to your Pi wallet.</p>`,
          });
        }
        if (seller?.email) {
          await sendPlatformEmail({
          to: seller.email,
            ...
            html: `<p>Order #${order.id.slice(0, 8)} dispute has been resolved in buyer's favour.</p><p>Refund amount: <strong>${amountPi.toFixed(2)}π</strong>.</p>`,
          });
        }
      } catch (emailErr) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: isConfigError
            ? "Pi payout not configured. Set PI_PAYOUT_API_URL and PI_PAYOUT_API_KEY (or PI_TREASURY_UID for direct Pi API)."
            : msg,
        },
        { status }
      );
    ...
    console.error("[Refund] Unexpected error:", outerErr);
    return NextResponse.json(
      { success: false, error: outerErr?.message ?? "Internal server error" },
      { status: 500 }
    );
  }
}
