...
// GET — check if current user follows this username (query: username=)
export async function GET(req: NextRequest) {
  try {
    const auth = req.headers.get("authorization")?.replace("Bearer ", "");
    if (!auth) return NextResponse.json({ success: true, data: { following: false } });
    const payload = verifyToken(auth);
    if (!payload) return NextResponse.json({ success: true, data: { following: false } });
    ...
    if (!target) return NextResponse.json({ success: true, data: { following: false } });
    ...
    return NextResponse.json({ success: true, data: { following: !!row } });
  } catch {
